...
export default function LeftPanel({
  ...
  React.useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const safeRole = typeof profileData?.role === 'string' ? profileData.role : 'Developer';
        const safeAvatar = typeof profileData?.avatar_url === 'string' ? profileData.avatar_url : null;
  ...
  // NOTE: Replace this with backend data later.
  const groups: any[] = [];

  // Helper: normalize group data so React never renders objects
  const normalizeGroup = (group: any) => {
    const id = String(group?.id ?? '');
    const name = String(group?.name ?? 'Unknown Group');
    const messagesCount = Array.isArray(group?.messages) ? group.messages.length : 0;
    return { id, name, messagesCount };
  };
      ...
      <div className="flex flex-col h-full">
          ...
          <div className="space-y-1">
            {groups.map((g) => {
              ...
              return (
                <Link
                  key={group.id}
                  href={`/messages?id=${group.id}`}
                  className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors text-left group ${
                    isDarkMode ? 'hover:bg-[#222]' : 'hover:bg-gray-50'
                  }`}
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                      isDarkMode
                        ? 'bg-[#222] text-gray-400 group-hover:bg-[#333]'
          ...
          <div
            className={`rounded-xl p-4 border ${
              isDarkMode
                ...
                : 'bg-gradient-to-br from-[#2ECC71]/10 to-[#2ECC71]/20 border-[#2ECC71]/20'
            }`}
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              <div>
                ...
                >
                  Upskill with top tech courses
                </p>
              </div>
            </div>
            <Link
              href="/magna-school"
              className={`block w-full py-2 rounded-lg text-xs font-bold shadow-sm transition-all text-center ${
                isDarkMode
                  ? 'bg-[#2ECC71] text-white hover:bg-[#25a25a]'
                  : 'bg-white text-[#2ECC71] hover:shadow-md'
              }`}
            >
              Start Learning
            </Link>
        ...
        <div className="mb-6">
          <div className="bg-gradient-to-br from-[#E50914]/5 to-[#F4A261]/10 rounded-xl p-4 border border-[#E50914]/10">
            ...
            <Link
              href="/get-verification"
              className="block w-full py-2 rounded-lg bg-[#E50914] text-white text-xs font-bold shadow-sm hover:bg-[#cc0812] transition-all text-center"
            >
          ...
          <Link href="/settings" className="w-full block">
            <div
              className={`flex items-center gap-3 px-3 py-2 rounded-xl transition-colors ${
                isDarkMode ? 'text-gray-400 hover:bg-[#222]' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              <Settings size={20} />
  ...
  return (
    <aside
      className={`w-[88px] h-screen fixed left-0 top-0 border-r flex flex-col z-20 hidden md:flex transition-all duration-300 overflow-y-auto pb-10 ${
    ...
    >
      <div className="p-6 relative">
        ...
        <Link
          href="/"
          className={`flex items-center gap-3 justify-center lg:justify-start transition-all duration-300 ${
            isSidebarExpanded ? 'mb-8' : 'mb-14'
          }`}
        >
        ...
        {setIsSidebarExpanded && (
          <button
            ...
            } ${isSidebarExpanded ? 'top-8 right-4' : 'top-20 left-1/2 -translate-x-1/2'}`}
          >
        ...
        {/* User Profile */}
        <div
          className={`flex items-center gap-3 p-0 rounded-xl mb-6 justify-center ${
            isSidebarExpanded
              ? isDarkMode
        ...
        >
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute bottom-0 right-0 w-3 h-3 bg-[#2ECC71] border-2 border-white rounded-full" />
          </div>

          <div className={`flex-1 min-w-0 hidden ${isSidebarExpanded ? 'lg:block' : ''}`}>
            <h3 className={`text-sm font-bold truncate ${isDarkMode ? 'text-white' : 'text-black'}`}>
              {typeof userName === 'string' ? userName : 'User'}
            </h3>
            <p className="text-xs text-gray-500 truncate">{typeof userRole === 'string' ? userRole : 'Developer'}</p>
          </div>
        </div>
          ...
          <Link href="/feed">
            <div
              className={`relative w-full flex items-center justify-center ${
                isSidebarExpanded ? 'lg:justify-between px-2 lg:px-4' : 'lg:justify-center px-0'
              } py-3 rounded-full transition-all text-sm font-medium cursor-pointer ${
                activeTab === 'Dashboard'
          ...
          <Link href="/magna-podcast">
            <div
              className={`relative w-full flex items-center justify-center ${
                ...
                activeTab === 'Magna Podcast'
                  ? 'bg-[#E50914] text-white shadow-md'
                  : isDarkMode
                    ? 'text-gray-400 hover:text-white hover:bg-white/10'
                    : 'text-gray-600 hover:bg-gray-100'
              }`}
              onClick={() => handleNavClick('Magna Podcast')}
            >
              <div className="flex items-center gap-3">
                <Mic size={20} />
        ...
        <div className={`mt-6 hidden ${isSidebarExpanded ? 'lg:block' : ''}`}>
          ...
          <div className="space-y-3">
            <Link
              href="/create-post"
              className="w-full py-2.5 px-4 rounded-full bg-gradient-to-r from-[#F4A261] to-[#E50914] text-white text-sm font-semibold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
            >
              <Plus size={18} />
              Create Post
            </Link>
        ...
        <div className={`mt-6 hidden ${isSidebarExpanded ? 'lg:block' : ''}`}>
          <div className="flex items-center justify-between mb-3 px-2">
            <h4 className="text-xs font-bold text-gray-400 uppercase">Your Groups</h4>
            <Link href="/messages?filter=groups" className="text-[#E50914] text-xs font-medium hover:underline">
              See All
            </Link>
          </div>
            ...
            {groups.map((g) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className={`text-sm font-medium truncate ${isDarkMode ? 'text-white' : 'text-black'}`}>
                      {group.name}
                    </h5>
          ...
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              <div>
                ...
                <p className={`text-xs leading-tight mt-0.5 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Upskill with top tech courses
                </p>
              </div>
            </div>
            <Link
              href="/magna-school"
              className={`block w-full py-2 rounded-lg text-xs font-bold shadow-sm transition-all text-center ${
                isDarkMode ? 'bg-[#2ECC71] text-white hover:bg-[#25a25a]' : 'bg-white text-[#2ECC71] hover:shadow-md'
              }`}
            >
              Start Learning
            </Link>
        ...
        <nav className="space-y-1">
          <Link href="/settings" className="w-full block">
            <div
              className={`relative w-full flex items-center justify-center ${
                ...
                isDarkMode ? 'text-gray-400 hover:text-white hover:bg-white/10' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              <div className="flex items-center gap-3">
                <Settings size={20} />
          ...
          <button onClick={handleLogout} className="w-full block text-left">
            <div className="relative w-full flex items-center justify-center py-3 rounded-full transition-all text-sm font-medium cursor-pointer text-[#E50914] hover:bg-[#E50914]/10">
              <div className="flex items-center gap-3">
                <LogOut size={20} />
      ...
      <div className={`px-2 mt-4 pb-8 flex flex-col items-center gap-4 ${isSidebarExpanded ? 'lg:hidden' : 'lg:flex'}`}>
        ...
        <Link
          href="/magna-school"
          className="w-10 h-10 rounded-xl bg-[#2ECC71]/10 text-[#2ECC71] hover:shadow-sm transition-all flex items-center justify-center"
        >
        ...
        <Link
          href="/get-verification"
          className="w-10 h-10 rounded-xl bg-[#E50914]/10 text-[#E50914] hover:shadow-sm transition-all flex items-center justify-center"
        >
          <BadgeCheck size={20} />
        </Link>
      </div>
